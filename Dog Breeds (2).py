...
def getDogsize(size):
    if size == "tiny":
        for i in range(len(minWeight)):
            if minWeight[i] < 11:
                filtered_list.append(dogList[i])
        print(filtered_list)
        breed = input("Which breed would you like?")
        Ipic = dogList.index(breed)
        open_image(images[Ipic])
        print("Some temperments of this dog are, " + temp[Ipic])

    if size == "small":
        for i in range(len(minWeight)):
            if minWeight[i] > 10 and minWeight[i] < 26:
                filtered_list.append(dogList[i])
        print(filtered_list)
        breed = input("Which breed would you like?")
        Ipic = dogList.index(breed)
    ...
    if size == "medium":
        for i in range(len(minWeight)):
            if minWeight[i] > 25 and minWeight[i]< 61:
                filtered_list.append(dogList[i])
        print(filtered_list)
        breed = input("Which breed would you like?")
        Ipic = dogList.index(breed)
...
def dog_menu():
    print("Welcome to, Pick Your Dog Breed!!")
    while True:
        ans= int(input("""1. Find your dog
                    2.Full list of dog breeds
                    3.Quit"""))
        if ans == 1:
            getDogsize(size=input("Please enter if you want a tiny, small, medium, or large dog."))
        if ans == 2:
            print(dogList)
        if ans == 3:
